...
describe 'eucalyptus::ntp', :type => :class do
...
#    end#opt_strings
  ...
  context "When on a RedHat-esque system" do
    ...
    context 'when fed no parameters' do
      ...
      it {
        should contain_package('ntp').with({:ensure=>"present"})
      }
      it {
        should contain_file('/etc/ntp.conf').with({:path=>"/etc/ntp.conf", :owner=>"root", :group=>"root", :mode=>"0644", :source=>"puppet:///modules/eucalyptus/ntp.conf"}).that_requires('Package[ntp]')
      }
      it {
        should contain_service('ntpd').with({:ensure=>"running", :enable=>true})
      }
    end#no params
  end
end
