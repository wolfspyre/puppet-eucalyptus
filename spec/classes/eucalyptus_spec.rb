...
describe 'eucalyptus', :type => :class do
  ...
  context "When on a RedHat system" do
    ...
    context 'when fed no parameters' do
      it {
        should contain_stage('before').that_comes_before('Stage[main]')
      }
      it {
        should contain_class('Eucalyptus::Repo').with({
          :stage=>"before",
      ...
      it {
        should contain_class('Eucalyptus::Security').with({ :stage=>"before"})
      }
    end#no params
  end
end
