...
describe 'eucalyptus::drbd_config', :type => :class do
...
#    end#opt_strings
  ...
  context "When on a RedHat system" do
    ...
    context 'when fed no parameters' do
      it { should contain_class('Eucalyptus::Drbd_config')}
      it {
        should contain_file_line('drbd master config entry').with({
          :ensure=>"present",
      ...
      it {
        should contain_package('drbd83-utils').with({
          :name=>"drbd83-utils",
          :ensure=>"present",
          :alias=>"drbd"
        })
      ...
      it {
        should contain_package('kmod-drbd83').with({
          :name=>"kmod-drbd83",
          :ensure=>"present",
          :alias=>"drbd-kmod"
        })
      ...
      it {
        should contain_eucalyptus__kern_module('drbd').with({
          :name=>"drbd",
          :ensure=>"present"
        })
      }
