...
describe 'eucalyptus::clc::install', :type => :class do
...
#    end#opt_strings
  ...
  context "When on a RedHat system" do
    ...
    it {
      should contain_package('eucalyptus-cloud').with({'ensure' => 'present'})
      should contain_service('eucalyptus-cloud').with({'ensure' => 'running', 'enable' => true})
    }
  end
end
