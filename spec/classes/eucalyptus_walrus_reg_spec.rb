...
describe 'eucalyptus::walrus::reg', :type => :class do
...
#    end#opt_strings
  ...
  context "When on a RedHat system" do
    ...
    let (:facts) {{'osfamily' => 'RedHat', 'operatingsystem' => 'redhat', 'hostname' => 'walrus_01', 'ipaddress_eth0' => '10.0.0.1'}}
    #TODO: set custom facts for eucakeys to permit testing of realization of exported file resources.
    let(:pre_condition) { ['Exec<||>','class{"eucalyptus::Clc": cloud_name => "cloud1", before =>"Class[Eucalyptus::walrus]" }', 'eucalyptus::cluster{"mycluster": cloud_name => "cloud1", cluster_name => "cluster1"}' ] }
    it 'should export the exec which fires euca_conf' do
      should contain_exec('reg_walrus_walrus_01').with({
        :command=>"/usr/sbin/euca_conf --no-rsync --no-scp     --no-sync --register-walrus --partition walrus     --host 10.0.0.1 --component walrus_walrus_01",
        :unless=>"/usr/sbin/euca_conf --list-walruses |     /bin/grep '\\b10.0.0.1\\b'",
        :tag=>"cloud1"
      })
    end
  end
