...
describe 'eucalyptus::walrus::install', :type => :class do
...
#    end#opt_strings
  ...
  context "When on a RedHat-esque system" do
    ...
    context 'when fed no parameters' do
      it do
        should contain_package('eucalyptus-walrus').with({
          :ensure=>"present"
        })
        should contain_service('eucalyptus-cloud').with({
          :ensure=>"running",
          :enable=>true
        })
      end
      context 'when the eucalyptus-cloud service is on this machine' do
        #TODO: currently this is done via an if!defined, so we'll place the
        #service in the catalog to simulate that behavior we should change this
        #to a better means of informing the classes of cohabitation
        let (:pre_condition){ ['service{"eucalyptus-cloud":}'] }
        it do
          should compile
        end
      end
